...
int XAxiDma_SimplePollExample()
{
	 ...
	 */
	CfgPtr = XAxiDma_LookupConfig(DMA_DEV_ID1);
	if (!CfgPtr) {
	...
	CfgPtr = XAxiDma_LookupConfig(DMA_DEV_ID3);
	if (!CfgPtr) {
		xil_printf("No config found for %d\r\n", DMA_DEV_ID3);
		return XST_FAILURE;
	}

	Status = XAxiDma_CfgInitialize(&AxiDma3, CfgPtr);
	if (Status != XST_SUCCESS) {
		xil_printf("Initialization failed %d\r\n", Status);
		return XST_FAILURE;
	}
	...
	for(Index = 0; Index < Tries; Index ++) {
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma1,(u32) TxBufferPtr,
